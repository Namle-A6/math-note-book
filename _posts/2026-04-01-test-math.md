...
**Ví dụ 5.2**: Cho một ma trận vuông cỡ 2x2. Ma trận vuông 2 \\\(\times\\\) 2 này có 2! = 2 phép thế cấp 2, đó là 
...
> Giả sử \\\(A = (a_{ij})_{n \times n} \\\) là một ma trận vuông cấp n với các phần tử là số thực hoặc số phức. Định thức của ma trận A, kí hiệu là \\\(det(A)\\\) hoặc \\\(|A|\\\) được cho bởi: 
